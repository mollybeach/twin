...
const AnalyticsComponent: React.FC<AnalyticsPropsType> = ({ analytics, twinId }) => {
  const { stateAllTwins, getTransactionTwinHistory, stateCurrentUserId } = useStore();
  const [error, setError] = useState<string | null>(null);
  const [transactions, setTransactions] = useState<TransactionType[]>([]);

  useEffect(() => {
    const fetchTransactions = async () => {
      if (!stateCurrentUserId) return;
      try {
        const fetchedTransactions = await getTransactionTwinHistory(stateCurrentUserId);
  ...
  }, [getTransactionTwinHistory, stateCurrentUserId]);

  const currentTransactions = transactions.filter((a: { twinId: string }) => a.twinId === twinId);
  const twin = stateAllTwins.find((a: { twinId: string }) => a.twinId === twinId);
  ...
  return (
    <div className="bg-white/10 backdrop-blur-lg rounded-lg p-6 space-y-8">
      ...
      <div className="space-y-4">
        ...
        {Array.isArray(currentTransactions) && currentTransactions.length > 0 && twin ? (
          <TradingChart
            twinId={twin.twinId}
